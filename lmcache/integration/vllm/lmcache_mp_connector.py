...
class LMCacheMPConnector(KVConnectorBase_V1):
    ...
    def __init__(
        ...
        if self.role == KVConnectorRole.SCHEDULER:
            ...
            # KV tunneling: durable manifest cache keyed by request_id,
            # holding ``(per_rank_manifest, first_block_id)``. Populated
            # once at first sight and pruned in ``request_finished`` — it
            # deliberately does NOT live on the request tracker, because
            # ``_get_or_create_request_tracker`` recreates the tracker on
            # PREEMPTED / async-load (WAITING_FOR_REMOTE_KVS) transitions
            # (see :1532), which would drop a tracker-attached manifest and
            # leave the forward-step snapshot empty → NaN at TP>1. Emitted
            # every step from here so the manifest is present in the
            # forward step's ``kv_connector_metadata`` regardless of
            # per-step scheduling or tracker churn.
            self._tunnel_manifests: dict[
            ...
            # KV tunneling: set once this worker first sees a tunnel
            # manifest. Lets ``handle_preemptions`` clear the worker
            # registry on later tunnel-free steps (so a finished request's
    ...
    def handle_preemptions(self, kv_connector_metadata: KVConnectorMetadata) -> None:
        ...
        binding ``kv_connector_metadata`` and the first
        ``_build_attention_metadata`` (see
        ``deps/vllm/vllm/v1/worker/gpu_model_runner.py:4036``); we
        repurpose it for kvtunnel registry-publish so the plugin doesn't
        ...
        rank under StreamingLLM). MLA collapses all TP-co-located ranks to
        ``kv_rank=0`` per ``extract_world_size_and_kv_rank``.
        """
        if not isinstance(kv_connector_metadata, LMCacheMPConnectorMetadata):
            return
        manifests = kv_connector_metadata.tunneled_manifests
        if not manifests and not self._tunnel_seen:
            # This worker has never seen a tunnel request. Skip the
            # kvtunnel import entirely so a pure-vLLM / stock-LMCache
            # deployment (no proxy, no tunnel) takes no kvtunnel
            # dependency — a half-installed kvtunnel plugin can still
    ...
    def _build_registry_snapshot(
        self,
        tunneled_manifests: dict[int, dict[int, TunneledRequestMetadata]],
    ) -> dict[int, TunneledRequestMetadata]:
        """Pick this rank's manifest out of the per-(fbid, rank) dict.

        Falls back to ``per_rank[0]`` for the rank-invariant single-rank
        case (most common; the pack emits the same manifest for every TP
        rank under StreamingLLM). MLA collapses all TP-co-located ranks
        to ``kv_rank=0`` per ``extract_world_size_and_kv_rank``.

        Returns:
            ``{first_block_id: manifest}`` for this rank.
        """
        ...
        snapshot: dict[int, TunneledRequestMetadata] = {}
        for first_block_id, per_rank in tunneled_manifests.items():
            manifest = per_rank.get(my_kv_rank)
            if manifest is None:
                manifest = per_rank.get(0)
            if manifest is None:
                continue
            snapshot[first_block_id] = manifest
        return snapshot
    ...
    def update_state_after_alloc(
        ...
        # KV tunneling: capture the manifest + first_block_id HERE, at
        # block allocation. For an async-KV-load tunnel request this fires
        # before its forward, so the manifest reaches the worker's per-step
        # snapshot in time at TP>1 (capturing at scheduled_new_reqs is one
        # engine step too late on vLLM 0.22.1 → empty snapshot at the
        # forward → magic ±inf header never dropped → NaN).
        self._capture_tunnel_manifest(
            request.request_id, tracker.kv_transfer_params, block_ids
        )
    ...
    def build_connector_meta(
        ...
        # so it is present in the forward step's metadata in time.
    ...
    def _capture_tunnel_manifest(
        self,
        request_id: str,
        kv_transfer_params: dict[str, Any] | None,
        block_ids: list[int],
    ) -> None:
        """Decode + cache a tunnel request's manifest, keyed by request_id.

        Stores ``(per_rank_manifest, first_block_id)`` in the durable
        ``self._tunnel_manifests`` cache. ``first_block_id = block_ids[0]``
        is the request's first allocated block — where chunk 0's magic
        header lands (rank-invariant).

        Called at block-allocation time (``update_state_after_alloc``),
        which for an async-KV-load tunnel request fires BEFORE its forward.
        Capturing here — rather than at ``scheduled_new_reqs`` — is what
        makes the manifest land in the worker's per-step snapshot in time
        at TP>1: on vLLM 0.22.1 the async-load request's forward build runs
        one engine step before it appears in ``scheduled_new_reqs``, so a
        capture there is too late (empty snapshot at the forward → magic
        ±inf header never dropped → NaN).

        Idempotent: a request already cached only has its ``first_block_id``
        refreshed (a preempt/resume may reallocate the first block). No-op
        for non-tunnel requests (no b64) and before any block is allocated.
        """
        if not block_ids:
            return
        ...
        if request_id in self._tunnel_manifests:
            manifest, _ = self._tunnel_manifests[request_id]
            self._tunnel_manifests[request_id] = (manifest, first_block_id)
            return
        try:
            ...
            # Narrow set: ValueError covers binascii.Error from b64decode;
            # msgspec.DecodeError covers malformed msgpack / type-shape
            # mismatches; TypeError covers non-bytes inputs. Anything else
            # is a real bug we crash on rather than silently drop tunneling.
            logger.exception(
    ...
    def _stage_tunneled_manifests(
        ...
        """Emit the per-step tunneling-manifest snapshot onto ``metadata``.

        Decoding + caching happens earlier in ``update_state_after_alloc``
        (at block allocation, BEFORE the request's forward); see
    ...
    def _cleanup_request_tracker(self, request_id: str) -> None:
        ...
        # Prune the durable tunnel-manifest cache (request finished, so it
        # must stop being emitted into the per-step snapshot).
        self._tunnel_manifests.pop(request_id, None)
