...
def _install_thread_pools(
    server: MessageQueueServer,
    all_specs: list[HandlerSpec],
    mp_config: MPServerConfig,
) -> None:
    """Register each spec's handler and partition handlers into thread pools.

    Affinity handlers (STORE/RETRIEVE) share the GPU-affinity pool; generic
    NORMAL handlers share one CPU pool; WAIT_STORE gets a DEDICATED normal
    pool (kvtunnel D2). Each WAIT_STORE handler parks its worker for the
    whole event.wait (up to ~30s backoff), so sharing the generic NORMAL
    pool would let it head-of-line-block MARSHAL. The ThreadPoolType enum
    carries no second-pool tag, so the compositor special-cases WAIT_STORE
    by request_type: exclude it from the generic NORMAL set, then add a
    second pool sized independently of max_cpu_workers (the canonical
    wrapper runs --max-cpu-workers 1) via KVTUNNEL_WAIT_STORE_WORKERS.
    """
    ...
    if RequestType.WAIT_STORE in {s.request_type for s in all_specs}:
        server.add_normal_thread_pool(
            [RequestType.WAIT_STORE],
            max_workers=int(
                os.environ.get(
                    "KVTUNNEL_WAIT_STORE_WORKERS",
                    max(8, mp_config.max_cpu_workers),
                )
            ),
        )
...
def run_cache_server(
    ...
    _install_thread_pools(server, all_specs, mp_config)
