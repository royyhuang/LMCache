...
class HandleTransferContext(TransferContext):
    ...
    def submit_retrieve(
        ...
        # kvtunnel: trailing marshal_handle (6th item), in lockstep with the
        # RETRIEVE ProtocolDefinition (protocols/engine.py). Empty when not
        # tunneled; the server then falls through to the stock lookup path.
        return self._send_request(
            self._mq_client,
...
class DataTransferContext(TransferContext):
    ...
    def submit_retrieve(
        ...
        # marshal_handle is GPU-tunnel-only; the non-GPU data path ignores
        # it (accepted to satisfy the TransferContext.submit_retrieve ABI
        # and the adapter's keyword call).
        del marshal_handle
        if self._non_gpu_context is None:
            raise RuntimeError(
                "Data transfer context is not registered. "
