...
class NonGPUTransferModule:
    ...
    def __init__(self, ctx: MPCacheServerContext) -> None:
        ...
        # Guards _non_gpu_contexts and _strategies together: they are
    ...
    def report_status(self) -> dict:
        ...
        with self._lock:
            entries = dict(self._non_gpu_contexts)
    ...
    def tracked_instance_count(self) -> int:
        ...
        with self._lock:
            return len(self._non_gpu_contexts)
    ...
    def reap_stale_instances(
        ...
        """
        now = time.monotonic()
        reaped: list[tuple[int, NonGPUContextEntry]] = []
        with self._lock:
            stale_ids = [
                iid
                for iid, entry in self._non_gpu_contexts.items()
            ...
            for iid in stale_ids:
                entry = self._non_gpu_contexts.pop(iid)
    ...
    ) -> tuple[NonGPUContextEntry, TransferStrategy]:
    ...
    def _release_entry(self, instance_id: int, entry: NonGPUContextEntry) -> None:
    ...
    def register_kv_cache_non_gpu_context(
        ...
        now = time.monotonic()
        with self._lock:
            existing = self._non_gpu_contexts.get(payload.instance_id)
        ...
        with self._lock:
            self._non_gpu_contexts[payload.instance_id] = entry
