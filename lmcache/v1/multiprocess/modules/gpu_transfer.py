...
class GPUTransferModule:
    ...
    def __init__(self, ctx: MPCacheEngineContext) -> None:
        ...
        # The kvtunnel MARSHAL workspace (ctx.marshal_workspace), the
        # GPU-context registry (ctx.gpu_context_registry), and the
        # WAIT_STORE chunk-commit notifier (ctx.chunk_commit_notifier) all
        # live on the shared context. MarshalModule constructs + publishes
        # the workspace; this module only reads it in the RETRIEVE
        # delegation and signals the notifier from the finish-write
        # callback below.
    ...
    def report_status(self) -> dict:
        ...
        for instance_id, entry in self._ctx.gpu_context_registry.items():
    ...
    def close(self) -> None:
        ...
        # Stop the drain thread (the thing that fires the finish-write
        # signal) before MarshalModule.close frees the workspace pool, both
        # before storage_manager.close(), so any in-flight completions reach
        # a live storage manager and the pinned host buffers their callbacks
        # touch are not unpinned out from under them. The ordering holds
        # because _build_modules appends MarshalModule AFTER this module and
        # MPCacheEngine.close iterates modules in list order.
        self._device_host_func_dispatcher.stop()

        had_contexts = len(self._ctx.gpu_context_registry.items()) > 0
        self._ctx.gpu_context_registry.clear()
        if had_contexts:
    ...
    def register_kv_cache(
        ...
        """
        if self._ctx.gpu_context_registry.get(instance_id) is not None:
            logger.warning(
        ...
        self._ctx.gpu_context_registry.register(
            instance_id,
            GPUContextEntry(
    ...
    def unregister_kv_cache(self, instance_id: int) -> None:
        ...
        self._ctx.gpu_context_registry.unregister(instance_id)
    ...
    # ----------------------------------------------------------------
    # WAIT_STORE signal side — the STORE-completion callback. The
    # WAIT_STORE handler itself lives in MarshalModule and waits on
    # ctx.chunk_commit_notifier; this callback (registered on this
    # module's DeviceHostFuncDispatcher) signals that notifier once the
    # L1 write completes. The single notifier.signal line below is the
    # intended residual STORE <-> WAIT_STORE coupling.
    # ----------------------------------------------------------------
    ...
    def _finish_write_and_signal(self, keys: list[ObjectKey]) -> None:
        ...
        try:
            ...
            self._ctx.chunk_commit_notifier.signal(chunk_hashes)
